...
<div class="bottomSide">
    ...
    const url = (url: string): string => /^(https?:)?\/\//g.exec(url) ? url : '//' + url;
    const notif = elemRoot.querySelector('.notification') as HTMLElement;

    const makeShort = (txt: string, length: number, mediaWidth?: number): string => {
        if (mediaWidth && window.matchMedia(`(max-width:${mediaWidth}px)`).matches)
            return txt.length > (length - 3) ? txt.substring(0, length - 3) + '...' : txt;
        return txt;
    };

    const error = (msg: string, time?: number): boolean => {
            notif.innerHTML = msg;
            notif.style.removeProperty('--startY');
            notif.style.removeProperty('--startOpacity');
            notif.style.setProperty('--time', time + "");
            notif.onanimationend = () => notif.style.removeProperty('display');
    ...
    const allGood = (embedObj: Embed): boolean => {
        let invalid, err;
        const str = JSON.stringify(embedObj, null, 4)
        const re = /("(?:icon_)?url": *")((?!\w+?:\/\/).+)"/g.exec(str);
        ...
        if (embedObj.timestamp && new Date(embedObj.timestamp).toString() === "Invalid Date") {
            ...
            if (!/\w+:|\/\/|^\//g.exec(re[2]) && re[2].includes('.')) {
                const activeInput = elemRoot.querySelector('input[class$="link" i]:focus') as HTMLInputElement;
                if (activeInput && activeInput.selectionStart) {
                    const lastPos = activeInput.selectionStart + 7;
    ...
    const timestamp = (stringISO?: string): string => {
    ...
    const display = (el: HTMLElement, data?: string, displayType?: string): void => {
        if (data) el.innerHTML = data;
        el.style.display = displayType || "unset";
    };

    const hide = (el: HTMLElement) => el.style.removeProperty('display');

    const externalParsing = ({ noEmojis, element }: { noEmojis?: boolean, element?: HTMLElement }): void => {
        // Uncomment the following line if you want to enable emoji parsing
        // if (!noEmojis) {
        //     twemoji.parse(element || elemRoot.querySelector('.msgEmbed'), { base: 'https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/' });
        // }
        // Highlight code blocks
        const codeBlocks = elemRoot.querySelectorAll('.markup pre > code');
        codeBlocks.forEach((block) => {
            hljs.highlightBlock(block as HTMLElement);
        });
    
        if (element) {
            const embed = element?.closest('.embed') as HTMLElement;
            if (embed && embed.innerText.trim()) {
                const targetElement = embed;
                targetElement.classList.remove('emptyEmbed');
            }
        }
    };

    const createEmbedFields = (fields: Field[], embedFields: HTMLElement): void => {
        ...
        for (const e of elemRoot.querySelectorAll('.embedField[style="grid-column: 1 / 5;"]') as NodeListOf<HTMLElement>)
            if (!e.nextElementSibling || (e.nextElementSibling as HTMLElement).style.gridColumn === '1 / 13')
    ...
    const embedContent = elemRoot.querySelector('.messageContent') as HTMLElement;
    const embedCont = elemRoot.querySelector('.msgEmbed>.container') as HTMLElement;
    const embedFragment = document.createDocumentFragment();
    embedFragment.appendChild((elemRoot.querySelector('.embed.markup') as HTMLElement).cloneNode(true));

    // Renders embed and message content.
    const buildEmbed = (jsonData?: DiscordEmbed, only?: string, index: number = 0): void => {
        if (jsonData) json = jsonData;
        if (!json.embeds?.length) elemRoot.classList.add('emptyEmbed');

        try {
            // If there's no message content, hide the message content HTML element.
            if (!json.content) elemRoot.classList.add('emptyContent');
            else {
                // Update embed content in render
                embedContent.innerHTML = markup(encodeHTML(json.content), { replaceEmojis: true });
                elemRoot.classList.remove('emptyContent');
            }
            ...
            for (const embedObj of embeds) {
                if (!allGood(embedObj)) continue;

                const embedElement = embedCont.appendChild(embedFragment.firstChild?.cloneNode(true) as HTMLElement) as HTMLElement;
                const embedGrid = embedElement.querySelector('.embedGrid') as HTMLElement;
                const msgEmbed = embedElement.querySelector('.msgEmbed') as HTMLElement;
                const embedTitle = embedElement.querySelector('.embedTitle') as HTMLElement;
                const embedDescription = embedElement.querySelector('.embedDescription') as HTMLElement;
                const embedAuthor = embedElement.querySelector('.embedAuthor') as HTMLElement;
                const embedFooter = embedElement.querySelector('.embedFooter') as HTMLElement;
                const embedImage = embedElement.querySelector('.embedImage > img') as HTMLImageElement;
                const embedThumbnail = embedElement.querySelector('.embedThumbnail > img') as HTMLImageElement;
                const embedFields = embedElement.querySelector('.embedFields') as HTMLElement;
                ...
                if (embedObj.color) (embedGrid.closest('.embed') as HTMLElement).style.borderColor = (typeof embedObj.color === 'number' ? '#' + embedObj.color.toString(16).padStart(6, "0") : embedObj.color);
                else (embedGrid.closest('.embed') as HTMLElement).style.removeProperty('border-color');
                ...
                const pre = embedGrid.querySelector('.markup pre') as HTMLElement;
                if (embedObj.thumbnail?.url) {
                    embedThumbnail.src = embedObj.thumbnail.url;
                    embedThumbnail.parentElement?.style.setProperty('display', 'block', 'important');
                    if (pre) pre.style.maxWidth = '90%';
                } else {
                    hide(embedThumbnail.parentElement as HTMLElement);
                    if (pre) pre.style.removeProperty('max-width');
                }

                if (embedObj.image?.url) {
                    embedImage.src = embedObj.image.url;
                    embedImage.parentElement?.style.setProperty('display', 'block', 'important');
                }
                else hide(embedImage.parentElement as HTMLElement);
                ...
                elemRoot.classList.remove('emptyEmbed');
                externalParsing({});
            ...
            if (e instanceof TypeError) {
                ...
                console.error(e);
                error('An error occurred while rendering the embed.');
            }
        }
    }

    const time = elemRoot.querySelector('.timeText') as HTMLElement;
    if (time) time.innerText = timestamp();

    for (const block of elemRoot.querySelectorAll('.markup pre > code'))
...
function loadMarkup(): void {
    const elems = document.getElementsByClassName("markup") as HTMLCollectionOf<HTMLElement>;
